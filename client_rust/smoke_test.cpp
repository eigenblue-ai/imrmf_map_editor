...
bool wait_for_yaml(std::string *out, int attempts = 50) {
  for (int i = 0; i < attempts; ++i) {
    *out = take(imrmf_client_snapshot_yaml());
    if (!out->empty())
      return true;
...
int main(int argc, char **argv) {
  ...
  std::printf(
      "load: %s\n",
      take(imrmf_client_load_building(server.c_str(), id.c_str())).c_str());

  std::string ws = server;
  if (ws.rfind("http://", 0) == 0)
    ws = "ws://" + ws.substr(7);
  ws += "/ws/" + id;
  std::printf("connect: %s\n", take(imrmf_client_connect(ws.c_str())).c_str());
  ...
  std::printf("synced=%d dirty=%d\nyaml:\n%s\n", imrmf_client_is_synced(),
              imrmf_client_remote_dirty(), yaml.c_str());

  const std::string edited = yaml + "\nsmoke_test_marker: true\n";
  std::printf("push: %s\n",
              take(imrmf_client_push_yaml(edited.c_str())).c_str());
  std::this_thread::sleep_for(std::chrono::milliseconds(500));

  const std::string after = take(imrmf_client_snapshot_yaml());
